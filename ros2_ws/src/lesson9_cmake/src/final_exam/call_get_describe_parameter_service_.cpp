...
void FinalExam::call_get_describe_parameter_service_(std::vector<std::string> velocity_name_vec) {
    ...
    while (this->describe_parameters_client_ptr_->wait_for_service(std::chrono::seconds(1)) == false) {
    ...
    auto future = this->describe_parameters_client_ptr_->async_send_request(request);
